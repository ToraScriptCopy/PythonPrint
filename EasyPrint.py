...
class Application(TkinterDnD.Tk):
    ...
    def _build_ui(self):
        lp = i18n.DATA[self.current_lang]
        self.sidebar = ctk.CTkFrame(self.main_container, width=300, corner_radius=0)
        self.sidebar.grid(row=0, column=0, sticky="nsew")
        ctk.CTkLabel(self.sidebar, text=lp["queue"], font=("Segoe UI", 20, "bold")).pack(pady=20)
        ...
        self.file_scroll = ctk.CTkScrollableFrame(self.sidebar, label_text=lp["drop_hint"])
        ...
        self.main_view = ctk.CTkFrame(self.main_container, fg_color="#0f0f0f", corner_radius=15)
        self.main_view.grid(row=0, column=1, sticky="nsew", padx=20, pady=20)
        self.pv_title = ctk.CTkLabel(self.main_view, text=lp["preview"], font=("Segoe UI", 14))
        ...
        self.ctrl_panel = ctk.CTkFrame(self.main_container, width=280)
        self.ctrl_panel.grid(row=0, column=2, sticky="nsew", padx=10, pady=20)
        ctk.CTkLabel(self.ctrl_panel, text=lp["params"], font=("Segoe UI", 16, "bold")).pack(pady=15)
        self.printer_cb = self._add_control(lp["printer"], PrinterService.get_available_printers())
        self.paper_cb = self._add_control(lp["paper"], ["A4", "Letter", "A3", "10x15"])
        ctk.CTkLabel(self.ctrl_panel, text=lp["copies"]).pack(anchor="w", padx=20)
        self.copy_input = ctk.CTkEntry(self.ctrl_panel)
        self.copy_input.insert(0, "1")
        self.copy_input.pack(pady=5, padx=20, fill="x")
        self.fit_check = ctk.CTkCheckBox(self.ctrl_panel, text=lp["fit"])
        self.fit_check.select()
        self.fit_check.pack(pady=20, padx=20)
        self.execute_btn = ctk.CTkButton(self.ctrl_panel, text=lp["print_btn"], height=65,
    ...
    def _add_file_to_list(self, path):
        ...
        if len(self.file_entries) == 1: self._update_preview(path)
    ...
    def _update_preview(self, path):
        try:
            ...
            self.preview_display.configure(image=None, text="Preview Error")

    def _start_printing(self):
        targets = [e.file_path for e in self.file_entries if e.is_active.get()]
        if not targets: return
        cfg = {'printer': self.printer_cb.get(), 'copies': self.copy_input.get()}
